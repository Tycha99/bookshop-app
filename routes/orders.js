...
// Список всех заказов (для админа) — ДОЛЖЕН БЫТЬ ВЫШЕ!
router.get('/all', requireRole('admin'), async (req, res) => {
    try {
        const [orders] = await db.query(`
            SELECT o.id, o.created_at, u.username, SUM(oi.quantity) as total_items
            FROM orders o
        ...
        res.status(500).send('Ошибка при получении всех заказов: ' + error.message);
    }
});
